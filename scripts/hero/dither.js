...
for (let x = 0; x < W; x++) { seed(x, 0); seed(x, 1); }          // topo
for (let y = 0; y < H * 0.55; y++) { seed(0, y); seed(1, y); seed(W - 1, y); seed(W - 2, y); } // laterais (só metade de cima; embaixo a camiseta toca a borda)
while (queue.length) {
  const i = queue.pop();
  const x = i % W, y = (i / W) | 0;
  const nb = [];
...
const grid = new Float32Array(GW * GH);   // luminância 0..1 (fundo = 0)
...
// 3) Níveis automáticos no sujeito + gama para clarear + fade na base.
...
const GAMMA = +(process.env.GAMMA || 0.8);
...
for (let gy = 0; gy < GH; gy++) for (let gx = 0; gx < GW; gx++) {
  ...
  v = Math.pow(v, GAMMA);
  v = 0.06 + 0.94 * v;             // piso: até as partes mais escuras do sujeito têm alguns pontos
  v *= alpha[i];
  const fadeStart = GH * (1 - FADE);
  if (gy > fadeStart) v *= Math.max(0, 1 - (gy - fadeStart) / (GH - fadeStart)) ** 1.5;
  tone[i] = v;
}

// 3b) Realce local (unsharp mask) para destacar óculos, olhos e barba.
const SHARP = +(process.env.SHARP || 0);
if (SHARP > 0) {
  ...
  for (let i = 0; i < GW * GH; i++) if (alpha[i] > 0) tone[i] = Math.min(1, Math.max(0, tone[i] + SHARP * (tone[i] - blur[i])));
}
...
let onCount = 0; for (let i = 0; i < on.length; i++) onCount += on[i];
console.log(`grade ${GW}x${GH}, pixels acesos: ${onCount}`);

// Saída: máscara + preview PNG (2x)
fs.writeFileSync(OUT, JSON.stringify({ GW, GH, rows: Array.from({ length: GH }, (_, y) => Array.from(on.subarray(y * GW, (y + 1) * GW)).join('')) }));
const SC = 2;
const prev = new PNG({ width: GW * SC, height: GH * SC });
for (let y = 0; y < GH * SC; y++) for (let x = 0; x < GW * SC; x++) {
  const v = on[((y / SC) | 0) * GW + ((x / SC) | 0)];
  const o = (y * GW * SC + x) * 4;
  prev.data[o] = v ? 0xA7 : 0x0A; prev.data[o + 1] = v ? 0x8B : 0x10; prev.data[o + 2] = v ? 0xFA : 0x1F; prev.data[o + 3] = 255;
}
fs.writeFileSync(OUT.replace(/\.json$/, '.png'), PNG.sync.write(prev));
// preview da máscara de fundo (para depuração)
const bgp = new PNG({ width: W, height: H });
for (let i = 0; i < W * H; i++) { const v = bg2[i] ? 0 : lum[i]; bgp.data[i * 4] = bgp.data[i * 4 + 1] = bgp.data[i * 4 + 2] = v; bgp.data[i * 4 + 3] = 255; }
fs.writeFileSync(OUT.replace(/\.json$/, '-bg.png'), PNG.sync.write(bgp));
